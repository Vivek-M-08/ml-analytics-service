...
def ingest_from_spec(spec_str, program_unique_id):
    ...
    if input_source["type"] == "local":
        ...
        print(f"⚠️ Unknown inputSource type: {input_source['type']}")
        return None
...
if __name__ == "__main__":
    print("🚀 Running re-check ingestion (auto-detect mode from spec)")

    program_ids = set(read_program_ids(PROGRAM_FILE))
    print(f"Found {len(program_ids)} program IDs in {PROGRAM_FILE}")
    ...
    for datasource in DATASOURCES:
        ...
        print(f"Total program IDs in datasource: {len(datasource_programs)}")

        missing_in_ds = program_ids - datasource_programs
        if missing_in_ds:
            print(f"❌ Missing in {datasource}: {sorted(missing_in_ds)}")

            for pid in missing_in_ds:
                resp = ingestion_map[datasource](pid)
                if resp and resp.status_code == 200:
                    print(f"✅ Ingestion started for {pid} in {datasource}")
                else:
                    status = resp.status_code if resp else "N/A"
                    text = resp.text if resp else "Invalid ingestion spec"
                    print(f"⚠️ Failed ingestion for {pid} in {datasource}, {status}: {text}")
        else:
            print(f"✅ All program IDs from {PROGRAM_FILE} are present in {datasource}")